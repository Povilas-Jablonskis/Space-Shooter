...
void display(void)
{
	...
	if (application->getState() == GameState::STARTED)
	{
		...
		renderObjects.insert(renderObjects.end(), player->getBulletsList()->begin(), player->getBulletsList()->end());
		renderer->draw(renderObjects);
		renderObjects.clear();
		...
		renderer->draw(listOfAllTexts, application->getFont("kenvector_future_thin"));
