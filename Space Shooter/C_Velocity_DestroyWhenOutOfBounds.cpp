...
void C_Velocity_DestroyWhenOutOfBounds::update(float deltaTime)
{
    ...
    if (m_owner->m_transform->getPosition().y > windowHeight || m_owner->m_transform->getPosition().y + height < 0.0f || m_owner->m_transform->getPosition().x > windowWidth || m_owner->m_transform->getPosition().x + width < 0.0f)
    {
...
void C_Velocity_DestroyWhenOutOfBounds::set(float x, float y)
{
    m_velocity.x = x;
    m_velocity.y = y;
}
