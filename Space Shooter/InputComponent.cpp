...
namespace Engine
{
	void InputComponent::update(Entity* entity, std::shared_ptr<InputManager> inputManager, float dt)
	{
		...
		if (inputManager->getKey("Move Left") || inputManager->getKey("Move Back") || inputManager->getKey("Move Right"))
		{
			...
			entity->addAddon(addon("leftExhaust", std::move(exhaust)));
			...
			entity->addAddon(addon("rightExhaust", std::move(exhaust)));
		}
